...
//	shared_ptr<string const> source = make_shared<string const>("TODO");//srcString.substr(secondColon));
//	int start = stoi(srcString.substr(0,firstColon));
//	int end = stoi(srcString.substr(firstColon, secondColon));
	int start = 0;
	int end = 3;
	shared_ptr<string const> source = make_shared<string const>("TODO");
...
ASTPointer<PragmaDirective> ASTJsonImporter::createPragmaDirective(Json::Value const& _node) //2REVIEW
{
	...
	{
                literals.push_back(lit.asCString()); //fixtype ?WHAT did i mean here?
                scanSingleToken(lit);
	}
	SourceLocation const& location = getSourceLocation(_node);
